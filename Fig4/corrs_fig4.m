...
for_5ePrism = cell2mat(within(1,2:5)); % move over later
...
print('Fig4\corrs_byOdor.pdf', '-dpdf', '-r600'); % -r600 sets 600 DPI resolution

% Alternatively, save as a high-resolution image (e.g., TIFF)
print('Fig4\corrs_byOdor.tif', '-dtiff', '-r600');
...
for i = 1:5
    ...
    prism_grab = cell2mat(corr_by_odor);
    for_5ePrism{i,1} = prism_grab(:);
...
% compressed to one matrix; every 6 rows is a single day; 
for_prism = cell2mat(vertcat(within_by_day{:}));
...
% Print the figure to a high-resolution PDF
print('Fig4\corrs_byBest.pdf', '-dpdf', '-r600'); % -r600 sets 600 DPI resolution

% Alternatively, save as a high-resolution image (e.g., TIFF)
print('Fig4\corrs_byBest.tif', '-dtiff', '-r600');


%% slightly different code for getting mean values (old prism doesn't like high n)
...
%   title(append('Day',num2str(i)));
    ...
    for j = 1:6
        ...
        across_loop = corr_now;
        across_loop = across_loop(~ismember(across_loop,within_loop));
        odor_pull{1,j} = mean(uniquetol(within_loop(within_loop<1)),'all');
        odor_pull{2,j} = std(uniquetol(within_loop(within_loop<1)),0,'all')...
            /sqrt(numel(uniquetol(within_loop(within_loop<1))));
        odor_pull_across{1,j} = across_loop;
    end
