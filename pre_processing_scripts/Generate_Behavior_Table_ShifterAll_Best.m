%% generate new tables compiled based on behavior cutoff for motion
clear
...
for k = 1:5
    ...
    responses_all = animal_sorted_bestCorrDay{1,k};
    Pre_delay_all = animal_sorted_delay{1,k};
    delay_all = animal_sorted_delay{1,k};
...
for k = 1:5
    ...
    for p = 1:length(responses_all)
        ...
        for i = 1:length(Odors_All)
            ...
            % code edits here to re-org behavior by best odor
            final_day{best_idx_now(i),p} = final_deltas(:,2);
            final_day_noseBEH{i,p} = sorted_nose_beh(:,2);
            final_day_motionBEH{i,p} = sorted_motion_beh(:,2);
            pre_final_day{best_idx_now(i),p} = final_deltas(:,1);
            ...
            Odor{i,1} = repmat(i,trial_cutoff,1);
        end
    end
...
%%
save('ProcessedData\tables_compiled_behaviorShifter_best.mat','tables_compiled');
