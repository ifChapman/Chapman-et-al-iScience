...
% set to false if you want to drop selected; true if you want to keep them
    ...
    for i = 1:length(Animals_All)
        ...
        for j = 1:length(Odors_All)
            ...
            for n = 1:max(OutputTable.Stim_TrialN)
                for_avg = zeros((max(OutputTable.Stim_TrialN)),inputValue.shifter_maxFrames);
                trial_forComp = prepped_for_corr{1,n};
                % if trial is not there, put an NaN in the shifter
                for o = 1:max(OutputTable.Stim_TrialN)
                    trial_forCorr = prepped_for_corr{1,o};
                    if isequal(trial_forCorr,trial_forComp)
                        for_avg(o,:) = NaN;
                    else
                        corr_pull = corr(trial_forComp,trial_forCorr);
                        corrTrace_now = max(corr_pull,[],2);
                        for_avg(o,:) = corrTrace_now;
                    end
                end
                % % standard trace method
                mean_trace = mean(for_avg,1,'omitnan');
                [max_meanTrace,idx] = max(mean_trace(1:window));
            ...
            for n = 1:max(OutputTable.Stim_TrialN)
                for_avg = zeros((max(OutputTable.Stim_TrialN)),inputValue.shifter_maxFrames);
                trial_forComp = Pre_prepped_for_corr{1,n};
                % if trial to check is not there, put an NaN in the shifter
                for o = 1:max(OutputTable.Stim_TrialN)
                    trial_forCorr = Pre_prepped_for_corr{1,o};
                    if isequal(trial_forCorr,trial_forComp)
                        for_avg(o,:) = NaN;
                    else
                        corr_pull = corr(trial_forComp,trial_forCorr);
...
animal_sorted_delay = cell(1,length(Days_All));
for d = 1:length(Days_All)
    ...
    delay_day_holder = cell(length(Animals_All),1);
    delay_day = Delay_byDay{1,d};
    responder_holder = cell(length(Animals_All),1);
    for i = 1:length(Animals_All)
        delay_now = delay_day{i,1};
        responders_now = day_now_responder{i,1};
        cells_now = day_now{i,1};
        responder_sorted = cell(1,length(Odors_All));
        odor_sorted = cell(1,length(Odors_All));
        delay_sorted = cell(1,length(Odors_All));
        corr_idx = corr_idx_D1{i,1};
        for t = 1:length(Odors_All)
            m = corr_idx(t);
            delay_idx = unique(OutputTable.Abs_TrialN(strcmp(OutputTable.Odor,Odors_All(m))==true & ... 
            OutputTable.Day == Days_All(d) & strcmp(OutputTable.Animal,Animals_All(i))==true,:));
            delay_sorted{1,t} = delay_now(delay_idx);
    ...
    delay_day_holder{i,1} = cell2mat(delay_sorted);
    day_holder{i,1} = cell2mat(odor_sorted);
    responder_holder{i,1} = cell2mat(responder_sorted);
    end
    animal_sorted_delay{1,d} = delay_day_holder;
