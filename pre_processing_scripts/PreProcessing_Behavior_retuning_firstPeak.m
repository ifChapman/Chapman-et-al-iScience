        ...
        for j = 1:length(Odors_All)
            ...
            for n = 1:max(OutputTable.Stim_TrialN)
                ...
                if (length(behavior_odor) > 0)
                    ...
                    for_pre = movmean(behavior_odor{n},12,'omitnan');
                    for_avg = fillmissing(for_avg,'linear');
                    % figure, findpeaks(for_avg(1501:1920),'MinPeakProminence',5)
                    [pks, locs] = findpeaks(for_avg(1501:1920),'MinPeakProminence',5);
                    if ~isempty(locs)
                        % figure,
                        % findpeaks(for_avg(1501:1920),'MinPeakProminence',10)
                        [~,idx] = min(abs(down_match-locs(1)));
                        % [~,idx] = min(abs(down_match-locs(find(pks == max(pks)))));
                        shifter_best_idx(1,n) = idx;
                    else
                        for_avg = movmean(motion_odor{n},12,'omitnan');
                        for_avg = for_avg(1501:1920);
                        if ~(sum(isnan(for_avg))==length(for_avg))
                            [pks,locs] = findpeaks(for_avg,'MinPeakProminence',1);
                            % figure, findpeaks(for_avg,'MinPeakProminence',1);
                            % if no eligible point, just get the peak
                            if isempty(locs)
                                [pks,locs] = max(for_avg,[],'omitnan');
                            end
                            % [~,idx] = min(abs(down_match-locs(find(pks == max(pks)))));
                            [~,idx] = min(abs(down_match-locs(1)));
                            shifter_best_idx(1,n) = idx;
...
save('PreProcessed_behaviorShifter_smoothed_retuned_firstPeak_021726.mat');
