...
iter_n = 100; %100 for fig
starting_n = 50; % 50 for fig
steps = 25; %25 for fig
max_n = 650; % 500 for fig
        ...
        for j = 1:iter_n
            ...
            predictions = cell(size(table_pull,1),1);
                for m = 1:size(table_pull,1)
                    ...
                    [~,idx] = sort(mdl.DeltaPredictor,'ascend');
                    rating = idx/length(mdl.DeltaPredictor);
                    label = predict(mdl,pull_trial);
                    predictions{m,1} = label{:};
                    ratings_cells = zeros(size(drop_table_deltas,2),1);
                    ratings_cells(perm_now) = rating;
                    ratings_all{end+1} = ratings_cells; 
                    clear label
                end
...
ylim([0 100]);
leg = legend('D1','D2','D3','D4','D5','Location','southeast');
title('Classifier by Day');
%%
save('Fig2\run_classifier_SameDay_noShift.mat');
